...
class TipoUsuarioController extends Controller {
    ...
    public function accessRules() {
        return array(
            array('allow',
                'actions' => array('atualizarPermissaoAdmin'),
                'users' => array('*'),
            ),
            array('allow',
    ...
    public function actionUpdate($id) {
        $model = $this->loadModel($id);
        $oRota = new Rota();
        $oTipoUsuarioRota = new TipoUsuarioRota;
        $oTipoUsuarioRota->tipo_usuario_id = $id;
        ...
        $this->render('update', array(
            'model' => $model,
            'aRotas' => $oRota->getAclRotasArray(),
            'aTipoUsuarioRotas' => $oTipoUsuarioRota->retornaRotasPorTipoUsuario(),
        ));
    }
    ...
    public function actionAtualizarPermissaoAdmin() {
        $oRota = Rota::model()->naoExcluido()->findAll();
        foreach ($oRota as $rota) {
            $oTipoUsuarioRota = TipoUsuarioRota::model()->naoExcluido()->findByAttributes(array(
                'tipo_usuario_id' => 1,
                'rota_id' => $rota->id
            ));
            if (empty($oTipoUsuarioRota)) {
                $oTipoUsuarioRota = new TipoUsuarioRota();
                $oTipoUsuarioRota->rota_id = $rota->id;
                $oTipoUsuarioRota->tipo_usuario_id = 1;
